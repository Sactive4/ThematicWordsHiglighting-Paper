...
params = []
# for n_estimators in tqdm([10, 50, 100]):
for n_estimators in tqdm([10, 20, 30, 50, 75, 100, 150]):
    for criterion in ["gini", "entropy", "log_loss"]:
        for max_depth in range(3, 40):
            params.append((n_estimators, criterion, max_depth))

for n_estimators, criterion, max_depth in tqdm(params):
    ...
    for threshold in threshold_array:  # Computing F1 for different threshold
        prediction = engine.round_nd_array(clf.predict_proba(X_test)[:, 1], threshold)
    ...
    if F1_score > best_F1_score:
        ...
        best_params = (criterion, max_depth, n_estimators)

print("MAX F1 =\t" + str(best_F1_score))
print("Best Params: \t\t" + "criterion = " + str(best_params[0]) + "\t\tmax_depth = " + str(
    best_params[1]) + "\t\tn_estimators = " + str(best_params[2]))
print("Best accuracy =\t" + str(best_accuracy))

clf = RandomForestClassifier(n_estimators=best_params[2], max_depth=best_params[1], criterion=best_params[0],
...
plt.savefig("data/" + "F1_score_function of a Random Forest V2.4.pdf", dpi=300)
plt.show()
